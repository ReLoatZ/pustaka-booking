...
class Dompdf
{
    ...
    */
    ...
    public function loadHtmlFile($file, $encoding = null)
    {
        ...
        $fileContent = Helpers::getFileContent($uri, $this->httpContext);
        $contents = $fileContent[0];
        $http_response_header = $fileContent[1];
        if (empty($contents)) {
            throw new Exception("File '$file' not found.");
        }
    ...
    private function processHtml()
    {
        ...
        if ($base_nodes->length) {
            ...
            $explode_url = Helpers::explode_url($href);
                $this->protocol = $explode_url[0];
                $this->baseHost = $explode_url[1];
                $this->basePath = $explode_url[2];
            }
